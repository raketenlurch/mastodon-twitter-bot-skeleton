...
def getTweets():
    ...
    if timeline is None:
        helpers._error('getTweets() => Could not retrieve tweets from the page. Please make sure the source Twitter account (' + url + ') is correct.')
        return False
...
def tootTheTweet(tweet):
    """ Receive a dictionary containing Tweet Id and text... and Toot!
        This function relies on the requests library to pst the content to your MMastodon account(human or bot).
        A boolean sucess status is returned.
        Arguments: tweet{dictionary} -- Dictionary containing the "id" and "text" of a single tweet.
    """
    ...
    if not host_instance:
        helpers._error('tootTheTweet() => Your Mastodon instance URL (' + host_instance + ') was incorrect.')
        return False

    if not token:
        helpers._config('tootTheTweet() => Your Mastodon access was incorrect.')
        return False
    ...
    if tweet_check_file.is_file():
        ...
        response = requests.post(url=host_instance + '/api/v1/statuses', data=data, headers=headers)
